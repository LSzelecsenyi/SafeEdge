...
class ProbabilityModelV3DevelopmentServiceTest {

	@Test
	void refusesLaLiga() {
		ProbabilityModelV3DevelopmentService service = new ProbabilityModelV3DevelopmentService(null);
		WalkForwardEvaluationRequest request = request(CanonicalCompetition.LA_LIGA);
		assertThatThrownBy(() -> service.diagnose(
						request, ProbabilityModelV3Config.defaults(), new BigDecimal("100000"), List.of(), null))
