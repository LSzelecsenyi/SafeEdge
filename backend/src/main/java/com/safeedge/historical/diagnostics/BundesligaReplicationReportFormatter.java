...
/**
 * Baseline 003 Bundesliga replication markdown. Zero-tuning. Not a betting
 * recommendation.
 */
public final class BundesligaReplicationReportFormatter {

	private BundesligaReplicationReportFormatter() {
	}
	...
	public static String format(
		...
		text.append("# Baseline 003 – Bundesliga Replication").append('\n').append('\n');
		...
		text.append('\n');
		text.append("The following sections reuse Baseline 002 edge-quality definitions on this Bundesliga dataset.")
				.append('\n')
				.append('\n');
	...
	private static void appendConfig(StringBuilder text, WalkForwardBuildStats stats) {
		...
		text.append("- No Bundesliga-specific home-advantage parameter.").append('\n');
		text.append("- Zero-tuning replication of Premier League Baseline 001/002.").append('\n');
	...
	private static void appendCoverage(StringBuilder text, LeagueCoverageSnapshot coverage) {
		...
		if (coverage.evaluationSeasonsComplete() && coverage.warmupSeasonsComplete()) {
			text.append("- All expected Bundesliga seasons are present, including 2021/22 (unlike Premier League).")
					.append('\n');
		}
		text.append('\n');
